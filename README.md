...
* Recommendation Engine: This system recommends restaurants on the basis of their locality, cuisines, famous food items, their ambience and their ctegory. This is done using `bag of words` and `cosine similarity`. It basically calculates the similarity score between two restaurants. After implementing it on the dataset, the similarity matrix was converted to a list which was then converted to a json file named [lcfa]
